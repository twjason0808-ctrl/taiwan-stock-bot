...
MIN_BUY_SCORE = 40
MARKET_BIAS_LONG_THRESHOLD = -5.0
...
def market_is_risk_off(market_bias_latest):
    if not market_bias_latest:
        return False
    bias = to_float(market_bias_latest.get("bias_55"))
    if pd.isna(bias):
        return False
    return bias < MARKET_BIAS_LONG_THRESHOLD


def market_risk_label(market_bias_latest):
    if not market_bias_latest:
        return "未知"
    return "禁多" if market_is_risk_off(market_bias_latest) else "可做多"


def derive_risk_decision(score, latest_price, stoploss, market_bias_latest, has_position=False):
    score_num = to_float(score)
    latest_num = to_float(latest_price)
    stop_num = to_float(stoploss)
    stoploss_hit = pd.notna(latest_num) and pd.notna(stop_num) and stop_num != 0 and latest_num < stop_num

    if stoploss_hit:
        return {
            "市場風控": market_risk_label(market_bias_latest),
            "大盤禁多": "是" if market_is_risk_off(market_bias_latest) else "否",
            "風控狀態": "強制出場",
            "執行動作": "立即出場",
            "風控原因": "跌破停損",
        }
    if market_is_risk_off(market_bias_latest):
        return {
            "市場風控": market_risk_label(market_bias_latest),
            "大盤禁多": "是",
            "風控狀態": "大盤偏空禁多",
            "執行動作": "禁止新倉" if not has_position else "禁止加碼，僅減碼或觀察",
            "風控原因": f"大盤 bias_55 < {MARKET_BIAS_LONG_THRESHOLD}",
        }
    if pd.notna(score_num) and score_num < MIN_BUY_SCORE:
        return {
            "市場風控": market_risk_label(market_bias_latest),
            "大盤禁多": "否",
            "風控狀態": "分數不足",
            "執行動作": "禁止新倉" if not has_position else "不加碼，續抱觀察",
            "風控原因": f"規則分數 < {MIN_BUY_SCORE}",
        }
    return {
        "市場風控": market_risk_label(market_bias_latest),
        "大盤禁多": "是" if market_is_risk_off(market_bias_latest) else "否",
        "風控狀態": "正常",
        "執行動作": "可觀察" if not has_position else "續抱／依策略管理",
        "風控原因": "未觸發強制風控",
    }
...
if not control.empty and "股票代號" in control.columns:
    ...
    control_risk = control.apply(
        lambda row: pd.Series(
            derive_risk_decision(
                score=row.get("規則分數"),
                latest_price=row.get("最新價"),
                stoploss=row.get("停損價"),
                market_bias_latest=market_bias_latest,
                has_position=False,
            )
        ),
        axis=1,
    )
    control = pd.concat([control, control_risk], axis=1)
...
if not trades.empty and "股票代號" in trades.columns:
    ...
    trades["規則分數"] = trades["股票代號"].map(lambda x: signal_map.get(x, {}).get("score", np.nan))
    trades["信號等級"] = trades["股票代號"].map(lambda x: signal_map.get(x, {}).get("signal", ""))
    trades["score_breakdown"] = trades["股票代號"].map(lambda x: signal_map.get(x, {}).get("score_breakdown", ""))
    trades["market_bias_latest"] = to_float(market_bias_latest.get("bias_55")) if market_bias_latest else np.nan
    trades["資料日期"] = now_ts
    ...
    trade_risk = trades.apply(
        lambda row: pd.Series(
            derive_risk_decision(
                score=row.get("規則分數"),
                latest_price=row.get("現價"),
                stoploss=row.get("停損價"),
                market_bias_latest=market_bias_latest,
                has_position=True,
            )
    ...
    trades = pd.concat([trades, trade_risk], axis=1)
...
for code in codes:
    ...
    row_payload.update(
        derive_risk_decision(
            score=row_payload.get("規則分數"),
            latest_price=row_payload.get("close"),
            stoploss=None,
            market_bias_latest=market_bias_latest,
            has_position=False,
        )
    )
    signal_rows.append(row_payload)
...
if market_bias_latest:
    ...
    summary_lines.append(f"市場風控模式：{market_risk_label(market_bias_latest)}")
    if market_is_risk_off(market_bias_latest):
        summary_lines.append(f"紀律規則：大盤 bias_55 < {MARKET_BIAS_LONG_THRESHOLD}，禁止新倉")
    summary_lines.append("")
...
if tracked.empty:
    ...
    for _, row in tracked.iterrows():
        summary_lines.append(
            f"- {row.get('股票代號', '')} {row.get('股票名稱', '') if '股票名稱' in tracked.columns else ''}｜現價 {row.get('最新價', 'N/A')}｜停損 {row.get('停損價', 'N/A')}｜距停損 {format_pct(row.get('距停損(%)'))}｜分數 {row.get('規則分數', 'N/A')}｜{row.get('信號等級', '')}｜{row.get('風控狀態', 'N/A')}｜{row.get('執行動作', 'N/A')}"
        )
summary_lines.append("")
...
if open_trades.empty:
    ...
    for _, row in open_trades.iterrows():
        summary_lines.append(
            f"- {row.get('股票代號', '')} {row.get('股票名稱', '') if '股票名稱' in open_trades.columns else ''}｜損益 {format_money(row.get('損益(元)'))} 元｜報酬 {format_pct(row.get('損益率'), assume_ratio=True)}｜{row.get('風控狀態', 'N/A')}｜{row.get('執行動作', 'N/A')}"
        )
summary_lines.append("")
...
blocked_new_entries = tracked[tracked["執行動作"] == "禁止新倉"] if (not tracked.empty and "執行動作" in tracked.columns) else pd.DataFrame()
if not blocked_new_entries.empty:
    summary_lines.append("新倉限制：")
    for _, row in blocked_new_entries.iterrows():
        summary_lines.append(f"- {row['股票代號']}｜{row.get('風控狀態', 'N/A')}｜{row.get('風控原因', 'N/A')}")
    summary_lines.append("")
...
if signals_new.empty:
    ...
    for i, (_, row) in enumerate(signals_new.head(10).iterrows(), start=1):
        summary_lines.append(
            f"- {i}. {row['股票代號']}｜分數 {row['規則分數']}｜{row['信號等級']}｜bias55 {format_pct(row.get('bias_55'))}｜突破 {format_pct(row.get('breakout_pct'))}｜量額 {row.get('volume_value_mil', 'N/A')}｜{row.get('執行動作', 'N/A')}"
        )
